...
-- Convites de assinatura por link público
CREATE TABLE IF NOT EXISTS contratos_assinatura_convites (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'pendente' CHECK (status IN ('pendente', 'acessado', 'assinado', 'expirado', 'cancelado')),
...
CREATE INDEX IF NOT EXISTS idx_contratos_assinatura_convites_user_id ON contratos_assinatura_convites(user_id);
...
-- Auditoria append-only do ciclo de vida do contrato
...
-- Partes e signatários do contrato (Fase 2)
...
CREATE TABLE IF NOT EXISTS contrato_parte_signatarios (
    ...
    status VARCHAR(30) NOT NULL DEFAULT 'pendente' CHECK (status IN ('pendente', 'convite_enviado', 'assinado', 'recusado', 'expirado')),
    data_cadastro TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW(),
    data_atualizacao TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT NOW()
);
...
ALTER TABLE contratos_assinatura_convites ADD COLUMN IF NOT EXISTS signatario_id VARCHAR(255) REFERENCES contrato_parte_signatarios(id) ON DELETE SET NULL;
